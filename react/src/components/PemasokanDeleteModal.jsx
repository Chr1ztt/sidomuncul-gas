...
export default function PemasokanDeleteModal({ closeDeleteModal, detail }) {
	...
	const onSubmitClick = (ev) => {
		...
		setLoading(true);
		axiosClient
			.delete(`/supply/${detail.id}`)
			.then(({ data }) => {
				setNotification("Riwayat pemasokan berhasil dihapus");
				closeDeleteModal();
				navigate(`/gudang/pemasokan`);
			})
		...
		<Backdrop onClick={(ev) => closeDeleteModal()}>
			...
			>
				...
				<div className="w-full justify-center items-center text-center py-2 text-sm sm:text-md ">
					Anda akan menghapus data pemasokan dengan pelanggan atas nama{" "}
					{detail.supplier.name}
					<br />
					Apakah Anda yakin?
          <br />
					<div className="text-xs py-3 px-5">
						*Peringatan : Data pemasokan yang belum tersimpan selama sehari tidak
