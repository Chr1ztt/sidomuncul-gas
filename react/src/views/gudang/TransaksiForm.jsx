...
export default function TransaksiForm() {
	...
	const onAddClick = () => {
		setList((prev) => {
			const newLine = { ...transactionLine, id: prev.length + 1 };
			return [...list, newLine];
		});
	...
	const onClearListClick = (li) => (ev) => {
		setList(list.filter((l) => l.id !== li.id));
		setTotalPrice((prev) => prev - li.gas_price);
	};
	...
	const onGasValueDropdownClick = (g) => {
		if (g.name.toLowerCase() !== transactionLine.gas_name?.toLowerCase()) {
			setTransactionLine({
				...transactionLine,
				tube_id:
					g.id === transactionLine.tube_gas_id ? transactionLine.tube_id : null,
				gas_id: g.id,
				gas_name: g.name,
			...
			<div className="flex flex-col w-full overflow-y-hidden pb-4">
					...
					<div className="flex flex-row w-full  sm:w-6/12  gap-2 ">
						<div className="flex w-6/12 ">
							...
							<DropdownMenu
								...
								loading={loadingGas}
								isSearchable={true}
								valueToBeSend={transactionLine.gas_name}
								// check={["gas_id", transactionLine.gas_id]}
								view={["name"]}
						...
						<div className="flex w-6/12 ">
							...
							<DropdownMenu
								...
								loading={loadingTube}
								isSearchable={true}
								valueToBeSend={transactionLine.tube_id}
								check={["gas_id", transactionLine.gas_id]}
								view={["id", "gas.name"]}
				...
				<div className="flex flex-col w-full">
					<div className="flex flex-row w-full justify-end font-extrabold  items-center">
						...
						<div className="w-3/12 text-end flex justify-end items-center rounded-r bg-white/80 py-2">
				...
				<div className="flex flex-row w-full justify-between mt-3 gap-2 items-center">
					<div className=" w-full max-w-[40%] rounded self-end">
						<input
							type="text"
					...
					<div className="w-full max-w-[6rem] flex flex-row self-end">
						<button
							onClick={onSaveClick}
							className="bg-forth mt-auto self-end hover:opacity-70  text-white  py-2 w-full max-w-[6rem] text-center rounded"
						>
